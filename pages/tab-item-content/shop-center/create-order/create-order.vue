<template>
	<view>
		<view >
			...
			style="margin-bottom:10px;padding-left:20px;border-bottom:1px solid #EEEEED;height:55px;line-height:55px;padding-right:16px;">
			...
			<view class="userInfo" >
				<view class="cu-form-group">
					...
					<input placeholder="请输入报修人" name="input" style="text-align:right;font-size:12px;" v-model="designer.name"></input>
				</view>

				<view class="cu-form-group">
					<view class="title"><text class="text-red">*</text>联系方式</view>
					<input placeholder="请输入联系方式" name="input" @blur="checkTelEvent($event)" style="text-align:right;font-size:12px;" v-model="designer.telephone"></input>
				</view>
		...
		onLoad(){
			...
			uni.getStorage({
				...
				success: (res) => {
					...
					this.designer.name=res.data.name;
					this.designer.telephone=res.data.account;
				}
			})
					...
					uni.getStorage({
						...
						success: (res) => {
							this.$ajax('NewServiceOrder',{
								...
								contractor:this.designer.name?this.designer.name:res.data.name,
								telephone:this.designer.telephone?this.designer.telephone:res.data.account
							},res=>{
								 uni.showToast({
								    title:'新增报修成功',
								    icon:'success'
