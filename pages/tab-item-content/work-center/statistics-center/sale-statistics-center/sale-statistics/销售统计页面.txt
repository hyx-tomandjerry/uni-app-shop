		...
		<view class="countNumber-container margin-bottom-normal bg-white">
			...
			<view class="aim-number justify-between flex"  style="padding:0 50px  24px 47px;">
				<view class="shop-count text-center">
					...
					<view class="font-size-middle font-weight-bold " style=" font-family: DINAlternate;">￥{{Number(shopfactPerform).toFixed(2) || 0}}</view>
				...
				<view class="alone-aim text-center">
					...
					<view class="font-size-middle font-weight-bold ">￥{{Number(shopAimPerform).toFixed(2) || 0}}</view>
				</view>
			</view>
		</view>
				...
				<view class="year-sale-detail-item align-center position_relative" v-for="(item,index) in salemanList"
					...
					<view style="width:100% ;padding-left:15px;"  >
						<view class="detail-num flex justify-between">
							<view  style="margin-top:6px;">
								<view class="font-weight-bold">{{item.field || ''}}</view>
								<view>占比 <text class="color-red">{{item.pre*100 || 0}}%</text></view>
							</view>
							<view class="font-family-num  font-size-middle font-weight-bold number-color">￥{{item.actual}}</view>
						</view>
						<view class="progress">
					...
					case 'year':
						this.$ajax('ShopYearlyPerformance',{
							...
							if(res){
								...
								this.shopAimPerform=res['expect'] || 0;
								this.chartData.series=[
									{
										name: '销售占比',
										data:this.shopAimPerform? Number(this.shopfactPerform/this.shopAimPerform).toFixed(2) :0,
										color: '#2fc25b',
									}
								];
								...
								if(res.salesmen.length>0){
									res.salesmen.forEach(item=>{
										item.pre=this.shopfactPerform?Number(item.actual/this.shopAimPerform*100).toFixed(2):0
									})
								}
					...
					case 'month':
						this.$ajax('ShopMonthlyPerformance',{
							...
							if(res){
								...
								if(res.salesmen.length>0){
									res.salesmen.forEach(item=>{
										item.pre=this.shopfactPerform?Number(item.actual/this.shopAimPerform).toFixed(2)*100:0
									})
								}
								this.salemanList=res.salesmen?res.salesmen:[];
								this.serverRingData.series=[];
					...
					case 'day':
						this.$ajax('ShopDailyPerformance',{
							...
							if(res){
								...
								if(res.salesmen.length>0){
									res.salesmen.forEach(item=>{
										item.pre=this.shopAimPerform?Number(item.actual/this.shopAimPerform).toFixed(2):0
									})
								}
								console.log(res.salesmen)
								this.salemanList=res.salesmen?res.salesmen:[];
								this.serverRingData.series=[];
								this.salemanList.forEach((item=>{
									this.serverRingData.series.push({
				...
				switch(this.timeTabVal){
						...
						this.$ajax('ShopYearlyPerformance',{
							...
							if(res){
								...
								this.chartData.series=[
									{
										name: '销售占比',
										data:this.shopAimPerform? (this.shopfactPerform/this.shopAimPerform).toFixed(2):0,
										color: '#2fc25b',
									}
								];
								this.showArcbar("canvasArcbar1",this.chartData);
								...
								for(var i =1;i<=12;i++){
									...
									this.showList.push({
										...
										name:`amonth${i}`,
										num:res[`amonth${i}`],
										pre:this.shopAimPerform?(res[`amonth${i}`]/this.shopAimPerform).toFixed(2):0
									})
					...
					case 'month':
						this.$ajax('ShopYearlyPerformance',{
							...
							this.shopfactPerform=res[`amonth${new Date().getMonth()+1}`];
							this.shopAimPerform=res[`month${new Date().getMonth()+1}`];
							this.$ajax('ShopMonthlyPerformance',{
								shop:this.shopID,
								year:this.timeObj.year,
								month:Number(this.timeObj.month)
								...
								if(res){
									...
									this.showList=[];
									this.performItem=res;
									console.log(this.shopfactPerform/this.shopAimPerform)
									this.chartData.series=[
										{
											name: '销售占比',
											data:this.shopAimPerform? Number(this.shopfactPerform/this.shopAimPerform).toFixed(2) :0,
									...
									for(var i =1;i<=days;i++){
										...
										this.showList.push({
											...
											name:`aday${1}`,
											num:res[`aday${i}`],
											pre:this.shopAimPerform?(res[`aday${i}`]/this.shopAimPerform).toFixed(2)*100:0
										})
									}
