	...
	<view>
		...
		<view style="padding-bottom:46px">
					...
					<view class="express-tab" style="margin-top:10px;background:orange">收</view>
						<view style="width:78%;">
							<view v-if="receiveShopItem || shopMaleInfo" style="padding-top:6px;">
								...
								<view style="font-size:12px;margin-top:10px;border-bottom:1px solid #EEEEED;padding-bottom:5px;" class="text-gray">{{receiveShopItem.provinceName|| shopMaleInfo.provinceName||''}}/{{receiveShopItem.cityName|| shopMaleInfo.cityName||''}}/{{receiveShopItem.districtName||shopMaleInfo.districtName ||''}}/{{receiveShopItem.address||shopMaleInfo.address||''}}</view>
							</view>
							<view  v-else style="padding-top:15px;min-height:55px;border-bottom: 1px solid #EEEEED;color:#aaa">请选择收件信息</view>
						</view>
					<text class="cuIcon-addressbook" 
					@click="choseReceive()"
					style="font-size: 30px;color:gray;position:absolute;right:10px;top:20px;" ></text>
			...
			<view class="express-info" style="padding:10px ;border:1px solid #EEEEED;border-radius: 10px;">
				...
				<view class="flex justify-between" style="border-bottom:1px solid #EEEEED;padding:10px;margin-top:6px" v-if="title.id==3">
					...
					<view>
						...
						<text class="cuIcon-right" style="font-size:15px;" @click="showModal($event)" data-target="RadioModal"></text>
					</view>
				</view>
				<view class="flex justify-between" style="border-bottom:1px solid #EEEEED;padding:10px;margin-top:6px" v-else>
					<view style="font-size: 13px;color:gray">
						<text style="color:red;margin-right:5px;">*</text>
			...
			<view class="goods-content" style="padding:10px 15px;color:gray;font-size:12px;border:1px solid #EEEEED;border-radius: 10px;margin: 10px 8px" >
				<view class="flex justify-between" v-if="title.id!=3">
				...
				<view class="goods-info" v-if="title.id!=3">
		...
		<view style="bottom:0px;left:0px;width:100%;position:fixed;background:lightgrey">
			<view class="flex justify-between">
				...
				<view class="cu-btn bg-orange">
					<text v-if="title.id==3" @click="createDistribute()">进行调拨</text>
					<text @click="createExpress()" v-else>立即下单</text>
				</view>
			</view>
		...
		methods: {
			createDistribute(){
				...
				this.$ajax('NewExpressTransfer',{
					...
					type:this.distributeObj.id,//调拨类型
			...
			createExpress(){
				...
				if(!this.isAccept){
					...
					if(!this.shopItem || !this.shopMaleInfo){
						...
						this.$ajax('NewProprietorExpress',{
							express:this.title.id,//订单类型（1是速递，2是物流，3是调拨)
							fromShop:this.shopItem.id,
							faddr:`${this.shopItem.provinceName || ''}${this.shopItem.cityName || ''}${this.shopItem.districtName || ''}${this.shopItem.address || ''}`,
							toShop:this.receiveShopItem.id?this.receiveShopItem.id:'',
							taddr:this.type==2?`${this.receiveShopItem.provinceName || ''}${this.receiveShopItem.cityName || ''}${this.receiveShopItem.districtName || ''}${this.receiveShopItem.address || ''}`:'',
		...
		onLoad(options){
			...
			if(options){
				if(options.type==1){
					...
					this.title={id:3,name:'调拨订单',value:'Transfer-order'}
				}
			}
			this.getUserInfo();
