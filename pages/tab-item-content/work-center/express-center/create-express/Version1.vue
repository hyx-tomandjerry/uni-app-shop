    ...
    <view>
        ...
        <view class="express-container borderTop bg-white">
            <view class="user-info border-top ">
                ...
                <view class="flex justify-start position_relative send-info borderBottom align-center">
                    ...
                    <view class="font-size-normal font-weight-normal shopInfo" >
                        <view v-if="receiveShop.provinceName">
        ...
        <view class="other-container bg-white" >
            <view class=" borderBottom" style="padding-top:16px;padding-bottom:17px;padding-left:13px;">
                <view style="margin-bottom:11px;"><text class="text-red">*</text><text class="font-size-normal">寄件类型</text></view>
        ...
        <view style="height:8px;width:100%;background:rgba(247,247,247,1)"></view>
        <view class="other-container bg-white" >
            ...
            <view class="goods-item flex justify-between borderBottom align-center" @click="showModal($event)" data-target="goodsInfo">
                ...
                <view style="width:75%;text-align:right;padding-top:2px;" class="text-ellipse">
                    <text class="font-weight-normal font-weight-normal text-grey" v-if="expressTypeValue || goods.summary">
                    ...
                    <text class="font-weight-normal font-weight-normal text-grey" v-else>
                        请选择包裹信息
                    </text>
            ...
            <view class="goods-item flex justify-between borderBottom align-center" @click="savePrice()">
                ...
                <view style="width:80%;text-align:right">
                    <input type="text" placeholder="未保价货品最高赔付9倍运费，建议保价"  class="font-size-normal font-weight-normal text-gray" v-model="goods.price"></view>
                <text class="cuIcon-right " style="font-size:18px;right:13px;top:30px;color:#898888;"></text>
            </view>
            <!--v-if="receiveTarget==expressItem.customer"-->
        ...
        <view style="padding-left:14px;height:36px;line-height:36px;background:rgba(247,247,247,1)" class="font-size-mini  align-center">
                ...
                <view class="bg-white">
                    <view  class="express-item-modal">
                        <view class="flex justify-start express-item-tab position_relative"
                            ...
                            <image src="../../../../../static/icon/icon-xuanzhong.png"
                                   v-if="item.id==expressTab" mode=""  class="position_absolute choose-item"></image>
                        </view>
                    </view>
                    ...
                    <view  class="bg-white nav">
                        ...
                        <view >
                            ...
                            <view class="flex justify-between time-info"
                                ...
                                {{item.name}}
                                <image src="../../../../../static/icon/icon-xuanzhong .png" style="width:18px;height:18px;"
                ...
                <view class="padding-xl borderTop bg-white">
                    ...
                    <view class="goods-info-item flex justify-between" style="margin-bottom:18px;">
                        ...
                        <view>
                            ...
                            <text style="font-size:19px;"><text style="color:#2A2A2A;padding:0 19px;">{{goods.weight}}</text> <text style="color:#898888;margin-right:4px;">kg</text></text>
                            <text class="num-tab tab-minus" @click="goodsNumOperate('weight','add')">+</text>
                        </view>
                    </view>
                    <view class="font-size-mini color-placeholder" style="white-space: nowrap;margin-bottom:14px;">
    ...
    export default{
        ...
        methods:{
            createExpress(){
                ...
                if(!this.isAccept){
                    ...
                    if(!this.sendShop ){
                        ...
                        if(this.mainTime.value=='today'){
                            var day=date.getDate()<10?"0"+date.getDate():date.getDate();
                            timeName=year+'-'+month+'-'+day;

                        }else if(this.mainTime.value=='tomorrow'){
                            var day=(date.getDate()+1)<10?"0"+(date.getDate()+1):(date.getDate()+1);
                            timeName=year+'-'+month+'-'+day;

                        }else if(this.mainTime.value=='second'){
                            var day=(date.getDate()+2)<10?"0"+(date.getDate()+2):(date.getDate()+2);
                            timeName=year+'-'+month+'-'+day;
                            // console.log(`${timeName}  ${this.smallTimeName}`)
                        }
                        this.$ajax('NewWaybill',{
                            ...
                            console.log(res)
                            uni.navigateTo({
                                url:"../success-send/success-send?way="+this.expressCatalog.express+'&id='+res
                            })
                        })
                    }
                ...
                // console.log(this.sendShop.id)
                // console.log(this.receiveShop.id,this.receiveTarget)
                // console.log(`${timeName}  ${this.smallTimeName}`)
                // console.log(this.expressTypeTabcur)
                // console.log(this.goods.weight,this.goods.summary)
                // console.log(this.goodsTypeTabCur)
                // return;
            ...
            getTimeInfo(){
                ...
                this.timeTab=[
                    {name:'今天',value:'today',id:0},
                    {name:'明天',value:'tomorrow',id:1},
                    {name:'后天',value:'second',id:2},
                ]
            },
        ...
        onLoad(options){
            ...
            this.$fire.on('address',result=>{
                ...
                // console.log(this.receiverMan,this.receiveShop)
            })
        },
    }
</script>
<style lang="less" >
    ...
    .btn-container{
        background:#fff;
        .submit-price-btn{
            width:65%;padding-top:19px;padding-left:15px;padding-bottom:19px;font-size:16px;color:#2A2A2A
        }
        .submit-btn{
