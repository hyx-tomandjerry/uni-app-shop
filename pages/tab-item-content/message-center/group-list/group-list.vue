	...
	<view>
		...
		<view class="group-content">
			<view v-show="type=='group'">
				<view class="group-content-list flex justify-start" v-for="(item,index) in groupList" :key="index" @click="chatToObj(item)">
			...
			<view v-show="type=='telephone'">
				<uni-collapse accordion="true">
				    <uni-collapse-item :title="(item.name || '未指定部门')+'    ('+item.members.length+')'"  v-for="(item,index) in telephoneList" :key="index">
				        <view style="padding: 30upx;" v-for="(member,ind) in item.members" :key="ind">
				            <image :src="member.headurl" style="width:25px;height:25px;border-radius: 50%;margin-right:10px;vertical-align: middle;"></image>{{member.name}}
				        </view>
				    </uni-collapse-item>
